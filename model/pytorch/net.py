...
from model.pytorch.basemodel import BaseModel
from model.pytorch.basenet import BaseNet
from model.pytorch.loss import WGANLoss, IDMRFLoss
from model.pytorch.layer import init_weights, PureUpsampling, ConfidenceDrivenMaskLayer, SpectralNorm
import numpy as np
...
from util.pytorch_utils import generate_mask
        ...
        if self.opt.pretrain_network is False:
            if self.opt.mask_type == 'rect':
                ...
                self.netD = GlobalLocalDiscriminator(3, cnum=opt.d_cnum, act=act,
                                                     spectral_norm=self.opt.spectral_norm,
                                                     g_fc_channels=opt.img_shapes[0]//16*opt.img_shapes[1]//16*opt.d_cnum*4,
                                                     l_fc_channels=opt.img_shapes[0]//16*opt.img_shapes[1]//16*opt.d_cnum*4)
